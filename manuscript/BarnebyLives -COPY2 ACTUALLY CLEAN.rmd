...
subs_batch <- c(1207, 2818, 2747, 2748, 2760:2770, 2782:2784, 2435, 2517, 1424, 
                2644, 2038, 2326, 2618)

data <- read.csv('data/test_data-2accession.csv', na.strings = "") %>% 
...
original_names <- c(1424, 2038, 2326, 2435, 2517, 2618, 2644, 2782:2784)

data <- data |>
  sf::st_drop_geometry() |>
  split_scientificName(sciName_col = 'Scientific_name') |>
  
  # for these records, we want to retain the originally entered information 
  mutate(
    POW_Family = if_else(Collection_number %in% c(2435, 2517), 'Ranunculaceae', POW_Family),
    POW_Genus = if_else(Collection_number %in% original_names, Genus, POW_Genus),
    POW_Epithet = if_else(Collection_number %in% original_names, Epithet, POW_Epithet),
  ) |> 
  select(-SpellCk.gGRP, -Match, -SpellCk.taxon_rank, -Genus, -Epithet,
...
write.csv(data, file = 'data/processed/cleaned_data-problemchildren.csv', row.names = FALSE)
```
...
processed <- read.csv('data/processed/cleaned_data-problemchildren.csv') %>% 
...
../process2final.txt  bash render_labels.sh collector='Reed'
```
