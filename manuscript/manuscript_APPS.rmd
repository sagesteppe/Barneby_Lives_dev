...
test_data <- read.csv('data/test_data.csv', na.strings = "") %>% 
  drop_na(longitude) %>% 
  drop_na(latitude) %>% 
  drop_na(date_digital) %>% 
  st_as_sf(coords = c('longitude', 'latitude'), crs = 4326) 

n_families <- test_data %>% 
  st_drop_geometry() %>% 
  group_by(family) %>% 
  count() %>% 
  nrow() # 74 families
...
n_sp_authors <- test_data %>% 
  st_drop_geometry() %>% 
  drop_na(authority) %>% 
  count() # 557 groups of authors

n_infra_sp_auths <- test_data %>% 
  st_drop_geometry() %>% 
  drop_na(authority1) %>% 
  group_by(binomial, infraspecies) %>% 
  count() %>% 
...
system.time({ # if neccessary convert coordinates in degrees minutes second to decimal degrees
...
time_political_grabber <- system.time({ # grab political information for collection
  political_grabber(data, y = 'Collection_number', path = p)
})
