...
# how many of my genera were outdated?
genus_incongruence <- data_p %>% 
  sf::st_drop_geometry() %>% 
  filter(POW_Genus != 'NOT FOUND') %>% 
...
epithet_diff <- data_p %>% 
  sf::st_drop_geometry() %>% 
  filter(Epithet != POW_Epithet, !Full_name %in% unique(genus_mismatch$Full_name )) %>% 
...
... XX were preferred by author.


the number of misspelled binomials were XX (% percent).
