...
## Usage
BarnebyLives is run entirely from within Rstudio. 
...
time_spell_check <- system.time({ # ensure appropriate spellings of the species
  sc_res <- spell_check(data$Binomial, path = p2tax)
})
...
It took roughly three minutes (`r sum( time_trials[time_trials$Group=='Local', 'elapsed'])`s) minutes to run all local steps of BarnebyLives, and a further 15 minutes (`r time_trials[time_trials$Function=='powo_searcher', 'elapsed']`s) to search Plants of the World Online, and `r time_trials[time_trials$Function=='directions_grabber', 'elapsed']` to search Google Maps and write directions to sites.
Nearly all of the local run time is attributable to the spatial operations (spatial: `r sum(tt_local[tt_local$Module=='Geospatial', 'elapsed'])`s, style: `r sum(tt_local[tt_local$Module=='Style', 'elapsed'])`, taxonomic: `r sum(tt_local[tt_local$Module=='Taxonomic', 'elapsed'])`). 
...
drop_na(data_p, Long_AutoFill_Flag) %>% 
  nrow() # 4 auto fill cases

drop_na(data_p, Lat_AutoFill_Flag) %>% 
...
# NEED TO WORK ON INPUT DATA, INCONSISTENT USE OF AUTHORSHIP IN THE FIRST COLUMN
data_p %>% 
  sf::st_drop_geometry() %>% 
  filter(Full_name != POW_Full_name) %>% 
  select(Collection_number, Full_name, Binomial, POW_Full_name) 
```
