...
record <- read.csv(file.path(local, 'SoS-ExampleCollection.csv')) |>
  ...
  dplyr::mutate(Project_name = 'Default Template')
...
`r associates_writer(record[['Associates']], full = TRUE)` \endgroup

\vspace{\fill}
\begingroup

`r collection_writer(record, date=FALSE)`

\begin{flushright}
`r record[['Date_digital_text']]`
\end{flushright}
\endgroup
\begingroup
\small
