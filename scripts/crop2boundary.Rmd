...
```{r public land survey system only for west the front range}


bound_nm <- data.frame(
  y = c(31, 31, 50, 50, 31), 
  x = c(-105, -125, -125, -105, -103)
) %>% 
  st_as_sf(coords = c('x', 'y'), crs = 4326) %>% 
  st_bbox() %>% 
  st_as_sfc()
...
trs <- st_join(township, section, .predicate = st_overlaps) %>% 
  mutate(TRS = paste0(TWNSHPLAB, ' S', FRSTDIVLAB)) %>% 
  select(TRS)
