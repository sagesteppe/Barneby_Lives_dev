...
spell_check <- function(x) {
  ...
  if (any(grep(x = cla$scientificName, pattern = binom, fixed = T))) {
    return(binom)
  } else{ # try and determine which piece is incorrect. 
    
    #subset datasets really quickly
    ...
    if (any(grep(x = gen_strings, pattern = paste0('^', genus, '$')))) {
      ...
      return(clean_genus_Tag)
    } else {
      possible_genus_Tag <- gen_strings[which.min(adist(genus, gen_strings))]
    }
    ...
    if (any(grep(x = spe_strings, pattern = paste0('^', species, '$')))) {
      ...
      possible_species_Tag <- spe_strings[which.min(adist(species, spe_strings))]
    }
    
    if (exists('clean_genus_Tag') & exists ('clean_species_Tag'))
    {
      return('Both the genus and species appear to be spelled correctly, but are not in my lookup table')
    } else{
      combos <- ls()[grep(ls(), pattern = 'Tag')]
      search_q <- combos[c(grep(combos, pattern = 'genus'), grep(combos, pattern = 'species'))]
      search_nom <- paste(unlist(mget(search_q)), collapse = " ")
      
      if(any(grep(x = cla$scientificName, pattern = search_nom, fixed = T))){
        return(search_nom)
      } else{
        return(paste0 (search_nom, 'We cannot figure this one out'))
      }
    }
  }
}
